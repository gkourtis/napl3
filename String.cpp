...
String toString(Obj o,bool onlyName=true,bool shortForRef=true,Word signalPos=-1){
	...
	if(o.isInt())
		...
		String name{classNotFound};uWord i;bool found=dict.revFindIn(o,name,i);
		obj Class=o.Class();
...
l:		SWITCH(Class)
				...
				CASE(classBitString) s<<"problem _classBitString";
				CASE(classUndefined)
					if(found) 
						s<<"Undefined:"<<name; 
					else 
...
/*
				case _classClass:{
					s<<keyOf(o,NAPL(Class:0x));
					classInfo &ci=*(classInfo*)(ptr)o.V();
					s<<"  ";
					s<<"token="<<std::hex<<ci.token<<std::dec<<" ";
					s<<"fatherClass="<<toString(o.Class())<<" ";
					s<<"sizeofHeader="<<ci.sizeofHeader<<" ";
					s<<"sizeofElement="<<ci.sizeofElement<<" ";
					s<<"sizeofData_MP="<<ci.sizeofData_MP<<" ";
					break;
				}
*/
/*
				...
				CASE(classInt) 
					if(o.isToken())
						...
						s<<std::dec<<o.w;
*/
				CASE(classDeferred) s<<"@"<<toString((obj)o.V().value());l0Show=true;
				CASE(classFloat) s<<o.datum_T<float>();
				CASE(classDouble) s<<o.datum_T<double>(); 
				CASE(classInt64) s<<std::dec<<o.datum_T<int64_t>();
				CASE(classAggregate)
...
l3:				for(Word i:a) if(isToken(i)) goto l1; 
				...
				DEFAULT
					if(Class!=classClass){
						...
						;//assert0(false,"Problem In toString");
			endSWITCH
